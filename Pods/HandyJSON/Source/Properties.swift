...
func getProperties(forType type: Any.Type) -> [Property.Description]? {
    if let nominalType = Metadata.Struct(type1: type) {
        return fetchProperties(nominalType: nominalType)
    } else if let nominalType = Metadata.Class(type1: type) {
        return nominalType.properties()
    } else if let nominalType = Metadata.ObjcClassWrapper(type1: type),
